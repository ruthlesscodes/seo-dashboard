...
export function CreditUsage({ variant = "bar" }: { variant?: Variant }) {
  ...
  const barColor = pct < 80 ? "bg-meridian-600" : pct < 90 ? "bg-warning" : "bg-danger";
  ...
  if (loading) {
    ...
    return <div className="h-8 min-w-[4rem] animate-pulse rounded-lg bg-meridian-100" />;
  }
      ...
      <div className={cn("flex items-center gap-2", variant === "bar" && "w-full")}>
        {variant === "bar" ? (
          <Progress.Root value={pct} className="h-2 flex-1 overflow-hidden rounded-full bg-meridian-100">
            <Progress.Indicator className={cn("h-full transition-[width] duration-700 ease-out", barColor)} style={{ width: `${pct}%` }} />
          </Progress.Root>
        ) : (
          <span className="text-sm font-medium tabular-nums text-ink-2">{remaining} / {limit}</span>
        )}
      </div>
      {variant === "bar" && (
        <span className="w-full text-left text-xs text-ink-3">Credits remaining — {remaining.toLocaleString()} / {limit.toLocaleString()}</span>
      )}
    </div>
  );
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          ...
          type="button"
        >
          {variant === "bar" ? content : <span className="text-sm font-medium tabular-nums text-ink-2">{remaining} / {limit}</span>}
        </button>
      </PopoverTrigger>
      <PopoverContent align={variant === "bar" ? "center" : "end"} className="w-64 animate-scale-in rounded-xl border border-meridian-100 bg-white p-4 shadow-lg">
        <div className="space-y-2">
          ...
          <p className="text-xs text-ink-3">{remaining.toLocaleString()} of {limit.toLocaleString()} remaining</p>
